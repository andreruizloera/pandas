...
- Fixed bug in :func:`read_csv` with the ``c`` engine where an integer key in ``dtype``, ``converters`` or ``na_values`` was applied both as a column label and as a column position when the column names were themselves integers (e.g. ``names=[1, 2]``), so one entry in the dict affected two columns; such a key is now a label only, matching ``engine="python"``. A key that names no column is still read as a position (:issue:`67005`)
